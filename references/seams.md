...
| 添加浏览器半设置面板 | 浏览器半注册 React 组件到 settings.section 等 slot；npm 面板优先 `ctx.connection.rpc.handle/call`（见 references/connection-rpc.md）；禁止裸 `@Remote` / 手写 manifest / `createRequire` 源码桥，完整 Typert `./typert` + `./remote` 仍合法 |
